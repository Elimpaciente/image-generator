...
async function handleRequest(request) {
  ...
  if (path === "/" || path === "") {
    return jsonRes("Prompt and model parameters are required")
  }
  if (path === "/models") {
    return new Response(
  ...
  if (path === "/generate") {
    ...
    if (prompt.length > 2000) {
      return jsonRes("Prompt must be less than 2000 characters")
    }
    ...
    try {
      ...
      return jsonRes("Error generating image. Please try again.")
    }
  }
  if (path === "/image") {
    if (request.method !== 'GET') {
